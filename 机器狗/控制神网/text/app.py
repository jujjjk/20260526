...
STATE_REFRESH_HZ = float(os.environ.get("RS04_STATE_REFRESH_HZ", "80.0"))
STATE_REFRESH_ENABLED = os.environ.get("RS04_STATE_REFRESH_ENABLED", "1").lower() in ("1", "true", "yes", "on")
STATE_REFRESH_WARMUP_FULL = os.environ.get("RS04_STATE_REFRESH_WARMUP_FULL", "1").lower() in ("1", "true", "yes", "on")

_spi_sequence_lock = threading.RLock()
_state_refresh_stop = threading.Event()
_state_refresh_thread: threading.Thread | None = None
_state_refresh_stats_lock = threading.Lock()
_state_refresh_stats = {
    "enabled": STATE_REFRESH_ENABLED,
    "target_hz": STATE_REFRESH_HZ,
    "ok_count": 0,
    "error_count": 0,
    "last_ok_ts": 0.0,
    "last_error_ts": 0.0,
...
def _refresh_all_states(full: bool = True):
    # A板代表：0x11；B板代表：0x31
    with _spi_sequence_lock:
        if full:
            motors[0x11].refresh_board_snapshot()
            motors[0x31].refresh_board_snapshot()
        else:
            motors[0x11].refresh_board_snapshot_once()
            motors[0x31].refresh_board_snapshot_once()


def _record_state_refresh_ok(elapsed_ms: float):
    with _state_refresh_stats_lock:
        _state_refresh_stats["ok_count"] += 1
        _state_refresh_stats["last_ok_ts"] = time.time()
        _state_refresh_stats["last_refresh_ms"] = float(elapsed_ms)


def _record_state_refresh_error(exc: Exception):
    with _state_refresh_stats_lock:
        _state_refresh_stats["error_count"] += 1
        _state_refresh_stats["last_error_ts"] = time.time()
        _state_refresh_stats["last_error"] = repr(exc)


def _state_refresh_loop():
    period = 1.0 / max(STATE_REFRESH_HZ, 1e-3)

    if STATE_REFRESH_WARMUP_FULL:
        try:
            start = time.perf_counter()
            _refresh_all_states(full=True)
            _record_state_refresh_ok((time.perf_counter() - start) * 1000.0)
        except Exception as exc:
            _record_state_refresh_error(exc)

    while not _state_refresh_stop.is_set():
        start = time.perf_counter()
        try:
            _refresh_all_states(full=False)
            elapsed = time.perf_counter() - start
            _record_state_refresh_ok(elapsed * 1000.0)
        except Exception as exc:
            elapsed = time.perf_counter() - start
            _record_state_refresh_error(exc)

        sleep_s = max(0.0, period - elapsed)
        if _state_refresh_stop.wait(sleep_s):
            break


@app.on_event("startup")
def _start_state_refresh_thread():
    global _state_refresh_thread
    if not STATE_REFRESH_ENABLED:
        return
    if _state_refresh_thread is not None and _state_refresh_thread.is_alive():
        return
    _state_refresh_stop.clear()
    _state_refresh_thread = threading.Thread(
        target=_state_refresh_loop,
        name="rs04_state_refresh",
        daemon=True,
    )
...
def _stop_state_refresh_thread():
...
@app.get("/api/state")
def api_state(motor_id: int | None = None, refresh: bool = False):
    if refresh:
        _refresh_all_states(full=True)
...
@app.get("/api/state_stats")
def api_state_stats():
    with _state_refresh_stats_lock:
        stats = dict(_state_refresh_stats)
    stats["thread_alive"] = _state_refresh_thread is not None and _state_refresh_thread.is_alive()
    stats["now"] = time.time()
    if stats["last_ok_ts"] > 0.0:
        stats["last_ok_age_ms"] = (stats["now"] - stats["last_ok_ts"]) * 1000.0
    else:
        stats["last_ok_age_ms"] = None
    return stats
