...
SPI_TX_DEBUG = os.environ.get("LINGZU_SPI_TX_DEBUG", "0").lower() in ("1", "true", "yes", "on")
...
class SPITransport:
    def __init__(self, targets=((0, 0), (1, 0)), max_speed_hz=1000000, frame_len=SPI_FRAME_LEN):
    ...
    def _get_spi(self, bus: int, dev: int):
        key = (int(bus), int(dev))
        spi = self._spis.get(key)
        if spi is not None:
            return spi
        spi = spidev.SpiDev()
        spi.open(bus, dev)
...
class LingZuMotorController:
    ...
    def __init__(self, motor_id=0x11, transport=None):
    ...
    def refresh_board_snapshot_once(self):
        """Read one cached board snapshot with a single SPI NOP frame."""
        target, _ = motor_to_target_and_port(self.motor_id)
