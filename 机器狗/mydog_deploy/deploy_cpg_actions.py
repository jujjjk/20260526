...
class DeployCPGCfg:
    ...
    phase_offsets: dict[str, dict[str, float]] = field(
        default_factory=lambda: {"trot": {"FR": 0.0, "RL": 0.0, "FL": 0.5, "RR": 0.5}}
    )
    freq_min: float = 0.8
    freq_max: float = 1.8
    k_freq: float = 3.0
...
class DeployCPGAction:
    ...
    def q_cpg(self, cmd: np.ndarray):
        freq = self._frequency(float(cmd[0]))
        ...
        for leg_i, leg in enumerate(self.cfg.leg_order):
            ...
            if phase01 < swing_fraction:
                ...
                stride = 1.0 - 2.0 * s
            base = leg_i * 3
            q[base] += moving * self.cfg.hip_amp * stride
    ...
    def target_policy(self, action_raw: np.ndarray, cmd: np.ndarray, action_mode: str, action_scale: np.ndarray) -> np.ndarray:
        action_clip = np.clip(np.asarray(action_raw, dtype=np.float32).reshape(12), -1.0, 1.0)
        ...
        self.last_q_cmd = (self.cfg.lowpass_alpha * q_limited + (1.0 - self.cfg.lowpass_alpha) * self.last_q_cmd).astype(np.float32)
        self.last_debug = {"q_cpg": q_cpg, "delta_q_rl": delta, "q_raw": q_raw, "q_cmd": self.last_q_cmd, "cpg_phase": phases, "cpg_frequency": freq}
