...
class UrdfConverter(AssetConverterBase):
    ...
    def _get_urdf_import_config(self) -> isaacsim.asset.importer.urdf._urdf.ImportConfig:
        ...
        # Consolidate links connected by fixed joints when requested.
        # Some Isaac Sim importer builds expose an additional
        # ``set_merge_fixed_ignore_inertia`` method while others do not.
        # Guard the optional call so the converter remains compatible across
        # importer versions.
